...
В некоторых случаях использование режима ``detailed mode`` позволяет уменьшить количество пропущенных пользователей до 10-20%, что на ~25-30% меньше, чем при использовании ``fast mode``.
